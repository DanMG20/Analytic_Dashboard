...
class DashboardDataProvider:
    ...
    def build_view_model(self) -> DashboardViewModel:
        ...
        stats_model, daily_models, video_models = (
            self.repository.get_all_dashboard_data()
        )

        stats_df = (
            pd.DataFrame([stats_model.model_dump()]) if stats_model else pd.DataFrame()
        )
        ...
        curr_views, prev_views, growth, curr_date, prev_date = (
            self._calculate_monthly_growth(daily_df)
        )
    ...
    def _calculate_monthly_growth(
        self, df: pd.DataFrame
    ) -> Tuple[int, int, float, date, date]:
        ...
        v_curr = int(
            temp_df[(temp_df["month"] == curr_m) & (temp_df["year"] == curr_y)][
                "views"
            ].sum()
        )
        v_prev = int(
            temp_df[(temp_df["month"] == prev_m) & (temp_df["year"] == prev_y)][
                "views"
            ].sum()
        )
